...
theurl <- getURL("https://bodovani.zimni-plavani.info/souteze.php?rok=2017",.opts = list(ssl.verifypeer = FALSE) )
...
events<-events[-c(1:5), ]
#events<-events[-c(nrow(events),nrow(events)-1,nrow(events)-2), ]

colnames(sw1)<-c("umisteni","s.c.","jmeno","rocnik","oddil","VS","trat","charakter","cas","body")
sw1<-sw1[-1, ]

#download data from web
theurl <- getURL("https://bodovani.zimni-plavani.info/soutez_vysl.php?id=3756",.opts = list(ssl.verifypeer = FALSE) )
tables <- readHTMLTable(theurl, header = TRUE, as.data.frame = TRUE)
#tables <- list.clean(tables, fun = is.null, recursive = FALSE)
#n.rows <- unlist(lapply(tables, function(t) dim(t)[1]))
...
startListJicin<-maxTratList[,c("jmeno","umisteni")]

startList<-merge(startListSlavkov,startListJicin,by = "jmeno", all = TRUE)
